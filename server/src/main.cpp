...
const char *g_config_file = "/home/zhangyang/YFTP/server/config.json";
...
void initLoggerSystem() {
    try {
        ...
        std::string log_file = ConfigManager::getInstance().get<std::string>(
            "log_file", "logs/yftp" + getCurrentTime() + ".log");
        ...
        std::size_t max_queue_size = ConfigManager::getInstance().get<int>(
            "max_asynclog_que_size", 8192);
        std::size_t thread_count = ConfigManager::getInstance().get<int>(
            "max_asynclog_worker_thread", 1);
        std::string async_log_file =
            ConfigManager::getInstance().get<std::string>(
                "async_log_file", "logs/async_yftp" + getCurrentTime() + ".log");
